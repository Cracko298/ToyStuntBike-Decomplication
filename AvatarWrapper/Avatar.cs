...
public class Avatar
{
	...
	public bool IsLoaded => Renderer.IsLoaded;
	...
	public void StartAnimation(Model skinnedModel, string animationKey, bool LoopAnimation)
	{
		...
		object tag = skinnedModel.Tag;
		SkinningData val = (SkinningData)((tag is SkinningData) ? tag : null);
		if (val == null)
		{
			throw new InvalidOperationException("This model does not contain a SkinningData tag.");
		}
		skinnedAnimationPlayer = new AnimationPlayer(val);
		AnimationClip val2 = val.AnimationClips[animationKey];
		skinnedAnimationPlayer.StartClip(val2);
		isLoopingAnimation = LoopAnimation;
		isAnimating = true;
	}
	...
	public void StartAnimation(CustomAvatarAnimationData animationData, bool LoopAnimation)
	{
		...
		currentXNACustomAnimation = new CustomAvatarAnimationPlayer(animationData.Name, animationData.Length, animationData.Keyframes);
	...
	public Avatar(AvatarDescription desc)
	{
		...
		Description = desc;
		Animation = new AvatarAnimation((AvatarAnimationPreset)0);
		Renderer = new AvatarRenderer(desc, true);
		Renderer.View = View;
	...
	public void Update(GameTime gameTime)
	{
		...
		case AnimationType.SkinnedModel:
			skinnedAnimationPlayer.Update(timeSpan, true, World);
			break;
		case AnimationType.Collada:
			currentColladaAnimation.Update(timeSpan, IsLoopingAnimation);
			break;
	...
	public void Draw()
	{
		...
		case AnimationType.SkinnedModel:
		{
			Matrix[] array2 = (Matrix[])(object)new Matrix[71];
			Matrix[] boneTransforms = skinnedAnimationPlayer.GetBoneTransforms();
			IEnumerable<ModelBone> enumerable = ((IEnumerable<ModelBone>)currentSkinnedModel.Bones).Where((ModelBone bone) => bone.Name.Contains("joint"));
			int num = -1;
			Vector3 val = default(Vector3);
			Quaternion val2 = default(Quaternion);
			Vector3 val3 = default(Vector3);
			foreach (ModelBone item in enumerable)
			{
				num++;
				string s = item.Name.Replace("joint", "");
				int num2 = int.Parse(s) - 1;
				if (num2 < 71)
				{
					((Matrix)(ref boneTransforms[num])).Decompose(ref val, ref val2, ref val3);
					Matrix val4 = Matrix.CreateFromQuaternion(val2);
					array2[num2] = val4;
				}
			}
			if (!IsPC)
			{
				Renderer.Draw((IList<Matrix>)array2, expression);
			}
			else
			{
				DrawWireframeBones(array2);
			}
			break;
		}
		case AnimationType.Collada:
		{
			Matrix[] array3 = (Matrix[])(object)new Matrix[currentColladaAnimation.BoneTransforms.Count];
			currentColladaAnimation.BoneTransforms.CopyTo(array3, 0);
			if (!IsPC)
			{
				Renderer.Draw((IList<Matrix>)array3, Expression);
			}
			else
			{
				DrawWireframeBones(array3);
			}
			break;
		}
		case AnimationType.CustomAnimationXNA:
