...
public class TriggerManager
{
	...
	public const int TRIGGER_EDIT_SELECTED = 4;
	...
	public void Update()
	{
		for (int i = 0; i < 512; i++)
		{
			if (m_Trigger[i].m_Id != -1)
			{
				...
				if (m_Trigger[i].m_State == Trigger.TRIGGER_STATE.SUSPENDED && m_Trigger[i].m_Position.X < Program.m_CameraManager3D.m_CameraPosition.X + 60f)
				{
	...
	public void Save()
	{
		...
		path = Path.Combine("C:\\XNA\\Bike\\Platformer1\\HighResolutionContent", path);
	...
	public void Load(StreamReader inFile)
	{
		...
		string text = $"Levels\\level{Program.m_App.m_Level}.dat";
