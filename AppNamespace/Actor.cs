...
public class Actor
{
	...
	public void CalcBounds2D(bool bUseAll, bool bUsePitch)
	{
		...
		{
			Vector3 val4 = Program.m_CameraManager3D.WorldToScreen(m_BoundsTmp[i] + zero);
			...
			while (((Enumerator)(ref enumerator)).MoveNext())
			{
				...
				try
				{
					while (((Enumerator)(ref enumerator2)).MoveNext())
					{
						...
						byte[] array = new byte[vertexStride * numVertices];
						current.VertexBuffer.GetData<byte>(array);
						for (int i = 0; i < array.Length; i += vertexStride)
						{
							float num = BitConverter.ToSingle(array, i);
							float num2 = BitConverter.ToSingle(array, i + 4);
							float num3 = BitConverter.ToSingle(array, i + 8);
