...
public class CameraManager3D
{
	...
	public void Update()
	{
		...
		case State.INGAME:
			if (!Program.m_App.m_bEditor)
			{
				...
				if (Program.m_PlayerManager.GetPrimaryPlayer().m_Avatar != null)
				{
					float num = 0f;
					m_CameraPositionTarget.X = Program.m_PlayerManager.GetPrimaryPlayer().m_Avatar.Position.X - CAMERA_POSITION_OFFSET_X;
					m_CameraPositionTarget.Y = Program.m_PlayerManager.GetPrimaryPlayer().m_Avatar.Position.Y + CAMERA_POSITION_OFFSET_Y + num;
					m_CameraLookAtTarget.X = Program.m_PlayerManager.GetPrimaryPlayer().m_Avatar.Position.X + CAMERA_TARGET_OFFSET_X;
					m_CameraLookAtTarget.Y = Program.m_PlayerManager.GetPrimaryPlayer().m_Avatar.Position.Y + CAMERA_TARGET_OFFSET_Y + num;
				}
				else
				{
					m_CameraPositionTarget.X = Program.m_PlayerManager.GetPrimaryPlayer().m_Position.X - CAMERA_POSITION_OFFSET_X;
					m_CameraPositionTarget.Y = Program.m_PlayerManager.GetPrimaryPlayer().m_Position.Y + CAMERA_POSITION_OFFSET_Y;
					m_CameraLookAtTarget.X = Program.m_PlayerManager.GetPrimaryPlayer().m_Position.X + CAMERA_TARGET_OFFSET_X;
					m_CameraLookAtTarget.Y = Program.m_PlayerManager.GetPrimaryPlayer().m_Position.Y + CAMERA_TARGET_OFFSET_Y;
				}
	...
	public void SpeedAdjust()
	{
		...
		Vector2 linearVelocity = Program.m_PlayerManager.GetPrimaryPlayer().m_Ragdoll.Body.LinearVelocity;
